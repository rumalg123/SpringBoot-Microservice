...
public class WishlistAnalyticsClient {
    ...
    public WishlistAnalyticsClient(
    ...
    public WishlistPlatformSummary getPlatformSummary() {
        return call(() -> get(BASE_URL + "/platform/summary", WishlistPlatformSummary.class));
    }

    public List<MostWishedProduct> getMostWished(int limit) {
        return call(() -> getList(BASE_URL + "/platform/most-wished?limit=" + limit, new ParameterizedTypeReference<>() {}));
    }
    ...
    private <T> T get(String url, Class<T> type) {
        try {
            ...
            throw new DownstreamHttpException(ex.getStatusCode(), "Wishlist analytics HTTP error: " + ex.getStatusCode().value(), ex);
        } catch (RestClientException ex) {
            throw new ServiceUnavailableException("Wishlist analytics service unavailable", ex);
        }
    }
    ...
    private <T> T call(Supplier<T> action) {
        var retry = retryRegistry.retry("analytics-wishlist-client");
        return Retry.decorateSupplier(retry, () ->
                circuitBreakerFactory.create("analytics-wishlist-client")
                        .run(action::get, throwable -> {
                            if (throwable instanceof RuntimeException re) throw re;
                            throw new ServiceUnavailableException("Wishlist analytics service unavailable", throwable);
                        })
        ).get();
    }
}
