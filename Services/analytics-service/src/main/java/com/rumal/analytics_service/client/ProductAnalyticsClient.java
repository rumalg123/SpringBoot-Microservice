...
@Component
public class ProductAnalyticsClient {
    ...
    public ProductPlatformSummary getPlatformSummary() {
        return call(() -> get(BASE_URL + "/platform/summary", ProductPlatformSummary.class));
    }

    public List<ProductViewEntry> getTopViewed(int limit) {
        return call(() -> getList(BASE_URL + "/platform/top-viewed?limit=" + limit, new ParameterizedTypeReference<>() {}));
    }

    public List<ProductSoldEntry> getTopSold(int limit) {
        return call(() -> getList(BASE_URL + "/platform/top-sold?limit=" + limit, new ParameterizedTypeReference<>() {}));
    }

    public VendorProductSummary getVendorSummary(UUID vendorId) {
        return call(() -> get(BASE_URL + "/vendors/" + vendorId + "/summary", VendorProductSummary.class));
    }
    ...
    private <T> T get(String url, Class<T> type) {
        try {
            ...
            throw new DownstreamHttpException(ex.getStatusCode(), "Product analytics HTTP error: " + ex.getStatusCode().value(), ex);
        } catch (RestClientException ex) {
            throw new ServiceUnavailableException("Product analytics service unavailable", ex);
        }
    }
    ...
    private <T> T call(Supplier<T> action) {
        var retry = retryRegistry.retry("analytics-product-client");
        return Retry.decorateSupplier(retry, () ->
                circuitBreakerFactory.create("analytics-product-client")
                        .run(action::get, throwable -> {
                            if (throwable instanceof RuntimeException re) throw re;
                            throw new ServiceUnavailableException("Product analytics service unavailable", throwable);
                        })
        ).get();
    }
}
