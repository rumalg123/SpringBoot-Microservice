...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<?> notFound(ResourceNotFoundException ex) {
        log.warn("Order resource not found: {}", ex.getMessage());
    ...
    public ResponseEntity<?> conflict(DuplicateResourceException ex) {
        log.warn("Order duplicate resource: {}", ex.getMessage());
    ...
    public ResponseEntity<?> unauthorized(UnauthorizedException ex) {
        log.warn("Order unauthorized request: {}", ex.getMessage());
    ...
    public ResponseEntity<?> serviceUnavailable(ServiceUnavailableException ex) {
        log.error("Order downstream service unavailable: {}", ex.getMessage(), ex);
    ...
    public ResponseEntity<?> validation(MethodArgumentNotValidException ex) {
        log.warn("Order request argument validation failed: {}", ex.getMessage());
    ...
    public ResponseEntity<?> csvExportFailure(CsvExportException ex) {
        log.error("CSV export failed: {}", ex.getMessage(), ex);
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(error(HttpStatus.INTERNAL_SERVER_ERROR, "Failed to generate CSV export"));
    }
