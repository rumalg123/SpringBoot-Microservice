...
@Component
public class VendorLifecycleIdempotencyFilter extends OncePerRequestFilter {
    private static final Logger log = LoggerFactory.getLogger(VendorLifecycleIdempotencyFilter.class);
    ...
    public VendorLifecycleIdempotencyFilter(
        ...
        this.keyPrefix = StringUtils.hasText(keyPrefix) ? keyPrefix : "vs:idem:v1::";
    }
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) {
        ...
        if (!isProtectedMutationPath(path)) return true;
        return !StringUtils.hasText(request.getHeader(keyHeaderName));
    }

    private boolean isProtectedMutationPath(String path) {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (StringUtils.hasText(current)) {
            ...
            if (current.startsWith(PENDING_PREFIX)) {
                if (!matchesPendingHash(current.substring(PENDING_PREFIX.length()), reqHash)) {
        ...
        if (!acquire(redisKey, reqHash)) {
            ...
            if (StringUtils.hasText(existing) && existing.startsWith(PENDING_PREFIX)
                    && !matchesPendingHash(existing.substring(PENDING_PREFIX.length()), reqHash)) {
                writeConflict(response, "Same idempotency key cannot be used with a different request payload");
                return;
            }
            writeConflict(response, "Request with the same idempotency key is still processing");
    ...
    private String buildRedisKey(HttpServletRequest request, String idemKey) {
        ...
        String scope = StringUtils.hasText(actor) ? actor.trim() : "internal";
        String path = normalizePath(request);
    ...
    private boolean acquire(String key, String reqHash) {
        try {
            ...
            log.warn("Vendor idempotency Redis acquire failed (fail-open) for key {}", key, ex);
            return true;
        }
    }
    ...
    private String get(String key) {
        try {
            ...
            log.warn("Vendor idempotency Redis lookup failed (fail-open) for key {}", key, ex);
            return null;
        }
    }
    ...
    private void storeDone(String key, String reqHash, ContentCachingResponseWrapper response) {
        try {
            ...
            log.warn("Vendor idempotency Redis completion write failed (fail-open) for key {}", key, ex);
        }
    }
    ...
    private boolean matchesPendingHash(String json, String reqHash) {
        try {
            @SuppressWarnings("unchecked")
            Map<String, Object> payload = objectMapper.readValue(json, Map.class);
    ...
    private void safeDelete(String key) {
        try {
            ...
            log.debug("Vendor idempotency Redis delete cleanup failed for key {}", key, ex);
        }
    }
