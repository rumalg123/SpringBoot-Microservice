...
@Component
public class CartMutationIdempotencyFilter extends OncePerRequestFilter {
    private static final Logger log = LoggerFactory.getLogger(CartMutationIdempotencyFilter.class);
    ...
    public CartMutationIdempotencyFilter(
        ...
        this.responseTtl = responseTtl == null ? Duration.ofHours(12) : responseTtl;
        this.keyHeaderName = StringUtils.hasText(keyHeaderName) ? keyHeaderName : "Idempotency-Key";
        this.keyPrefix = StringUtils.hasText(keyPrefix) ? keyPrefix : "cs:idem:v1::";
    }
    ...
    private boolean acquire(String key, String reqHash) {
        try {
            ...
            log.warn("Cart idempotency Redis acquire failed (fail-open) for key {}", key, ex);
            return true;
        }
    }
    ...
    private String get(String key) {
        try {
            ...
            log.warn("Cart idempotency Redis lookup failed (fail-open) for key {}", key, ex);
            return null;
        }
    }
    ...
    private void storeDone(String key, String reqHash, ContentCachingResponseWrapper response) {
        try {
            ...
            log.warn("Cart idempotency Redis completion write failed (fail-open) for key {}", key, ex);
        }
    }
    ...
    private void safeDelete(String key) {
        try {
            ...
            log.debug("Cart idempotency Redis delete cleanup failed for key {}", key, ex);
        }
    }
