...
export default function CartError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div className="min-h-[350px] flex flex-col items-center justify-center px-6 py-14 text-center">
      ...
      <p className="mb-1.5 text-lg font-bold text-ink">Cart Error</p>
      <p className="mb-5 max-w-[420px] text-sm text-muted">{error.message || "Something went wrong loading your cart."}</p>
